...
// Lê a porta serial no Windows e grava CSVs sincronizados,
...
// Extrai valor numérico depois de "Heart Rate:" / "Breath Rate:" / "Distance:"
...
// Detecta se a linha está no formato:
// tempo,total_phase,breath_phase,heart_phase,dist
// e se todos os 5 campos são números válidos.
static bool is_phase_csv_line(const char* line) {
    ...
    // Contar vírgulas — deve ter exatamente 4
    int commas = 0;
    ...
    while(tok) {
        ...
        char* endp = NULL;
        strtod(tok, &endp);
        if(endp == tok) return false; // não converteu nada
        count++;
...
int main(void){
    ...
    fprintf(fPhaseRaw, "# --- NOVO DATASET %s ---\n", timestamp);
    fflush(fHeart); fflush(fBreath); fflush(fPhaseRaw);

    setvbuf(fHeart,    NULL, _IOLBF, 0);
    setvbuf(fBreath,   NULL, _IOLBF, 0);
    setvbuf(fPhaseRaw, NULL, _IOLBF, 0);
    ...
    printf("Arquivos: heart_rate.csv, breath_rate.csv, phases_raw.csv\n");
    printf("===============================================\n");
    ...
    for(;;){
        ...
        for(DWORD i=0;i<nread;++i){
            ...
            if(c=='\n'){
                ...
                // Marca tempo atual do PC
                LARGE_INTEGER now; QueryPerformanceCounter(&now);

                // === NOVO: se for linha CSV "tempo,total_phase,..." salva crua ===
                if(is_phase_csv_line(lineBuf)) {
                    fprintf(fPhaseRaw, "%s\n", lineBuf);
                    printf("[PHASE] %s\n", lineBuf);
                }
                ...
                // Calibração t0 usando Distance (como antes)
                if(!have_t0 && extract_value_after_colon(lineBuf, "Distance", &val)){
                    t0 = now; have_t0 = true;
                }
                // fallback: primeira linha com Heart/Breath/Distance
                if(!have_t0){
                    float tmp=0.f;
