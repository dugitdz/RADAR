...
fprintf('\n================ RESULTADOS (HEALTH-RADAR vs POLAR) ================\n');
...
for d = 1:numel(DATASETS)
    ...
    % Imprime os tempos totais para conferência
    try
        A_temp = readmatrix(radar_path);
        t_radar_total = (A_temp(end, COL_T_MS) - A_temp(1, COL_T_MS)) / 1000;
    catch
        t_radar_total = NaN;
    end
    t_polar_total = tP(end);
    fprintf('\n-> [%s] Durações: Radar = %.1f s | Polar = %.1f s\n', ds.name, t_radar_total, t_polar_total);
    ...
    % Calcula e imprime as métricas (agora com MAPE)
    [RMSEB, MAEB, MAPEB, CORRB, nB] = metrics_vs_polar(hB_onP, hP);
    fprintf('%s | HEALTH-RADAR: RMSE=%.3f MAE=%.3f MAPE=%.2f%% CORR=%.3f N=%d\n', ...
        radar_label, RMSEB, MAEB, MAPEB, CORRB, nB);
        
    ax(d) = nexttile;
    hold(ax(d), 'on'); grid(ax(d), 'on');
    
    h1 = plot(ax(d), tP, hP, 'r-', 'LineWidth', 1.6);
    if okB
        h2 = plot(ax(d), tP, hB_onP_plot, 'k-', 'LineWidth', 1.4);
    else
        h2 = plot(ax(d), nan, nan, 'k-', 'LineWidth', 1.4);
    end
    ...
    if d < numel(DATASETS)
        ...
        xlabel(ax(d), 'Tempo (s)', 'FontSize', 11, 'FontWeight', 'bold');
    end
    
    if isempty(hPolarLegend)
        hPolarLegend = h1;
        hRadarLegend = h2;
    end
end

% Vincula os eixos X e Y
linkaxes(ax, 'xy');

% Trava o X em 250 segundos e o Y entre 60 e 110 bpm para todos
xlim(ax(1), [0 250]);
ylim(ax(1), [60 110]); 

lgd = legend([hPolarLegend hRadarLegend], {'POLAR H10', 'HEALTH-RADAR'}, ...
...
% --- FUNÇÃO DE MÉTRICAS ATUALIZADA (AGORA COM MAPE) ---
function [RMSE, MAE, MAPE, CORR, nValid] = metrics_vs_polar(h_onP, hP)
    m = isfinite(h_onP) & isfinite(hP);
    nValid = nnz(m);
    if nValid < 5
        RMSE = NaN; MAE = NaN; MAPE = NaN; CORR = NaN;
        return;
    end
    e = h_onP(m) - hP(m);
    RMSE = sqrt(mean(e.^2,'omitnan'));
    MAE  = mean(abs(e),'omitnan');
    MAPE = mean(abs(e ./ hP(m)), 'omitnan') * 100; % Calcula erro percentual
    ...
    for i = 1:nt
        ...
        if conf >= conf_thr
            if kmax > 1 && kmax < numel(Pb)
                y1 = log(max(Pb(kmax-1), eps));
                y2 = log(max(Pb(kmax  ), eps));
                y3 = log(max(Pb(kmax+1), eps));
                denom = (y1 - 2*y2 + y3);
                if abs(denom) > eps
